...
SCENARIO("Adding new tasks to a Project", "[project]")
{

    const std::string projectIdent = "TestProject";

    GIVEN("an empty Project object with identifier '" + projectIdent + "'")
    {

        Project project{projectIdent};

        REQUIRE(project.size() == 0);

        WHEN("adding a new Task with a unique identifier")
        {

            const std::string taskIdent1 = "Task1";
            Task &task1 = project.newTask(taskIdent1);

            THEN("the Project contains 1 Task")
            {
                REQUIRE(project.size() == 1);

                AND_THEN("getting the Task using its identifier returns the correct object")
                {
                    REQUIRE(&project.getTask(taskIdent1) == &task1);
                }
            }

            AND_WHEN("adding another Task with a different identifier")
            {

                const std::string taskIdent2 = "Task2";
                Task &task2 = project.newTask(taskIdent2);

                THEN("the Project contains 2 Tasks")
                {
                    REQUIRE(project.size() == 2);

                    AND_THEN("getting the new Task using its identifier returns the correct object")
                    {
                        REQUIRE(&project.getTask(taskIdent2) == &task2);
                    }
                }

                AND_THEN("getting the first Task still returns the correct object")
                {
                    const Task &retrievedTask = project.getTask(taskIdent1);
                    REQUIRE(retrievedTask == task1);
                }
            }

            AND_WHEN("adding a Task with an existing identifier")
            {

                THEN("the existing Task object is returned")
                {
                    Task &existingTask = project.newTask(taskIdent1);
                    REQUIRE(&existingTask == &task1);
                    REQUIRE(project.size() == 1); // Size remains the same
                }
            }
        }
    }
